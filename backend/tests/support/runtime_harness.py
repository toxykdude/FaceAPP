...
def seed(session) -> Dict[str, Any]:
    ...
    for i, (dt, amount, method) in enumerate(rows):
        tx = SalesTransaction(
            ...
            invoice_number=f"INV-HARNESS-{i}-{suffix}",
            transaction_date=dt,
        )
        session.add(tx)
        txs.append(tx)
...
def main() -> int:
    ...
    try:
        ...
        def get(path: str, params: dict) -> httpx.Response:
            return httpx.get(
                f"{BASE}{path}", params=params, headers=headers, timeout=5.0
            )
        ...
        check(
            "summary reversed -> 422", r.status_code == 422, f"status={r.status_code}"
        )
        r = get("/sales/dashboard", reversed_params)
        check(
            "dashboard reversed -> 422", r.status_code == 422, f"status={r.status_code}"
        )
        ...
        check(
            "summary totals (out-of-window excluded)",
            float(summ.get("total_revenue", -1)) == 350.0
            and summ.get("total_transactions") == 2,
        ...
        dash_total = round(
            sum(p.get("amount", 0) for p in dash.get("revenue_trend", [])), 2
        )
        check(
            "dashboard==summary totals agree (==350.0)",
