...
class TestMemberGetUpdateDelete:
    ...
    def test_get_member_not_found(self, client, auth_headers):
        ...
        response = client.get("/api/members/00000000-0000-0000-0000-000000000000", headers=auth_headers)
        assert response.status_code == 404
    
    def test_update_member(self, client, auth_headers, sample_member):
        """Test updating a member."""
        response = client.put(f"/api/members/{sample_member.id}", headers=auth_headers, json={
            "first_name": "Updated"
        })
        assert response.status_code == 200
        data = response.json()
    ...
    def test_delete_member(self, client, auth_headers, sample_member):
        ...
        response = client.delete(f"/api/members/{sample_member.id}", headers=auth_headers)
