...
def list_events(
    ...
    events = (
        query.order_by(AccessEvent.timestamp.desc())
        .offset(skip)
        .limit(limit)
        .options(
...
@router.post(
    "", response_model=AccessEventResponse, status_code=status.HTTP_201_CREATED
)
def create_event(
...
def get_today_recognized(
    ...
    events = (
        ...
        .filter(AccessEvent.member_id.isnot(None), AccessEvent.timestamp >= today_start)
    ...
    for evt in events:
        ...
        latest_membership = (
            ...
            .filter(Membership.member_id == evt.member_id)
            .order_by(Membership.end_date.desc())
            .first()
        )
        ...
        result.append(
            {
                ...
                "membership_plan": (
                    (active_membership or latest_membership).type
                    if (active_membership or latest_membership)
                    else None
                ),
                "membership_end": (
                    latest_membership.end_date.isoformat()
                    if latest_membership and latest_membership.end_date
                    else None
                ),
                "photo_url": f"/api/members/{mid}/photo",
                "last_seen": (
                    evt.timestamp.replace(tzinfo=timezone.utc).isoformat()
                    if evt.timestamp
                    else None
                ),
...
def get_expiring_today(
    ...
    for m in expiring:
        ...
        if not member:
            continue
        plan = (
            db.query(MembershipPlan).filter(MembershipPlan.id == m.plan_id).first()
            if m.plan_id
            else None
        )
