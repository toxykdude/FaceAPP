...
> Historical context (pre `admin-data-tools`): Backend report windows used fixed Colombia offsets and sales views showed browser-local dates without times.
...
> Historical context (pre `admin-data-tools`): The flow existed and was tested in `main` but was unavailable in the deployed environment.
