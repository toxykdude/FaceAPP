...
- [x] **C.5 RED — Settings export button**: add `frontend/src/test/SettingsExportDb.test.tsx`: admin-only Export DB button calls `settingsApi.exportDatabase()`. Run: `npm run test -- SettingsExportDb` → **fail**. Deps: none.
- [x] **C.6 GREEN — Settings UI + i18n**: modify `frontend/src/api/settings.ts` (`exportDatabase()` → blob); `frontend/src/pages/Settings/Settings.tsx` (System-tab "Export Database" button, admin-gated, triggers download); i18n `settings.exportDb`, `settings.exportDbHelp`, `settings.backupRemoteStatus` (es+en). Verify: `npm run test -- SettingsExportDb` → green. Deps: C.2, C.5.
